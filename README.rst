...
For example, this ``setup.py`` provides a ``$ python setup.py publish`` command, which creates a universal wheel (and sdist) and uploads your package to PyPi using `Twine <https://pypi.python.org/pypi/twine>`_, without the need for an annoying `setup.cfg` file. 
